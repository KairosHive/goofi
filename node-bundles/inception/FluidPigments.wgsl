/* goofi
{"doc": "Sixteen EEG features drive liquid pigments, morphing and color.\nInputs 1-8: normalized LZ; 9-16: normalized Higuchi dimension.", "tags": ["image", "simulation"], "inputs": [{"name": "features", "kind": "ARRAY"}], "state": ["dye"], "params": [{"group": "fluid", "name": "speed", "kind": "float", "default": 0.7, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "swirl", "kind": "float", "default": 1.0, "min": 0.0, "max": 3.0}, {"group": "fluid", "name": "color", "kind": "float", "default": 1.0, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "relief", "kind": "float", "default": 0.65, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "light", "kind": "float", "default": 1.15, "min": 0.1, "max": 3.0}, {"group": "fluid", "name": "flow_scale", "kind": "float", "default": 1.0, "min": 0.3, "max": 3.0}, {"group": "fluid", "name": "eddies", "kind": "float", "default": 1.0, "min": 0.0, "max": 3.0}, {"group": "fluid", "name": "stretch", "kind": "float", "default": 0.3, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "morph", "kind": "float", "default": 0.65, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "renewal", "kind": "float", "default": 0.006, "min": 0.0, "max": 0.04}, {"group": "fluid", "name": "detail", "kind": "float", "default": 1.0, "min": 0.2, "max": 3.0}, {"group": "fluid", "name": "palette_shift", "kind": "float", "default": 0.0, "min": -1.0, "max": 1.0}, {"group": "fluid", "name": "color_spread", "kind": "float", "default": 1.0, "min": 0.2, "max": 2.0}, {"group": "fluid", "name": "saturation", "kind": "float", "default": 1.0, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "refraction", "kind": "float", "default": 1.0, "min": 0.0, "max": 3.0}, {"group": "fluid", "name": "gloss", "kind": "float", "default": 1.0, "min": 0.0, "max": 2.0}, {"group": "fluid", "name": "roughness", "kind": "float", "default": 0.35, "min": 0.05, "max": 1.0}, {"group": "fluid", "name": "light_angle", "kind": "float", "default": 0.0, "min": -3.14, "max": 3.14}, {"group": "fluid", "name": "contrast", "kind": "float", "default": 1.0, "min": 0.4, "max": 2.0}, {"group": "fluid", "name": "eeg_motion", "kind": "float", "default": 1.0, "min": 0.0, "max": 3.0}, {"group": "fluid", "name": "eeg_color", "kind": "float", "default": 0.35, "min": 0.0, "max": 2.0}]}
*/
fn eeg(i:i32)->f32 {
 let d=vec2i(textureDimensions(features));
 if d.x*d.y<16 {return 0.5;}
 let j=i%16;
...
fn noise(v:vec2f)->f32 {
 let a=floor(v);let f=fract(v);let w=f*f*(3.0-2.0*f);
 return mix(mix(hash(a),hash(a+vec2f(1,0)),w.x),mix(hash(a+vec2f(0,1)),hash(a+vec2f(1,1)),w.x),w.y);
}
fn fbm(v:vec2f)->f32 {
 var q=v;var s=0.0;var a=0.55;
 for(var i=0;i<5;i++){s+=a*noise(q);q=vec2f(q.x*1.63-q.y*1.17,q.x*1.17+q.y*1.63)+3.7;a*=0.48;}
 return s;
}
...
fn flow(uv:vec2f)->vec2f {
 ...
 let t=time*0.08;
 for(var i=0;i<8;i++){
  let k=i;let fi=f32(i);
  let centre=vec2f(0.5+0.39*sin(fi*2.399+(eeg(k)-0.5)*2.8*p.eeg_motion+t*0.17),
  ...
  v+=0.006*p.eddies*vec2f(f*sin(uv.x*f+phase)*cos(uv.y*f-phase),
                  -f*cos(uv.x*f+phase)*sin(uv.y*f-phase));
 }
 v+=p.stretch*0.12*vec2f(sin(uv.y*7.0+eeg(5)*3.0),cos(uv.x*6.0+eeg(13)*3.0));
 return v/aspect;
}
fn readDye(uv:vec2f)->vec4f {return textureSampleLevel(dye,samp,clamp(uv,vec2f(0.001),vec2f(0.999)),0.0);}
fn next_dye(uv:vec2f)->vec4f {
 if frame==0u {return seed(uv);}
...
fn palette(x:f32)->vec3f {
 ...
 if t<3.0{return mix(c,d,smoothstep(2.0,3.0,t));}
 if t<4.0{return mix(d,e,smoothstep(3.0,4.0,t));}
 if t<5.0{return mix(e,f,smoothstep(4.0,5.0,t));}
...
fn shade(uv:vec2f)->vec4f {
 ...
 let h=heightAt(uv);
 let grad=vec2f(heightAt(uv+vec2f(px.x,0))-heightAt(uv-vec2f(px.x,0)),
                 heightAt(uv+vec2f(0,px.y))-heightAt(uv-vec2f(0,px.y)))/px;
 let n=normalize(vec3f(-grad*p.relief*0.23,1.0));
 let view=normalize(vec3f((uv-0.5)*0.35,1.0));
 let l=normalize(vec3f(-0.45*cos(p.light_angle)-0.55*sin(p.light_angle),-0.45*sin(p.light_angle)+0.55*cos(p.light_angle),0.9));
