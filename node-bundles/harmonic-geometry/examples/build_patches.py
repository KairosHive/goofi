...
class Patch:
    def __init__(self, slug, title, auto=True, mix=0.3):
        ...
        self.nodes, self.links, self.names, self.variables = {}, [], {}, []
        self.views = []
        self.canvas = False
        self.monitor = None
        self.row = 0
        ...
        for name, value in [('default_ufreq', 20.0), ('default_width', 256), ('default_height', 256)]:
            self.variables.append({'name': 'system.'+name, 'type': 'int' if isinstance(value, int) else 'float', 'value': value})
        ...
        self.bind('slow', 'lfo', 'amplitude', '0.5 if variables.geometry.auto else 0.0')
        self.bind('slow', 'lfo', 'offset', '0.5 if variables.geometry.auto else variables.geometry.mix')
    ...
    def control(self, name, value, lo=0.0, hi=1.0, kind='slider', options=None, step=None):
        ...
        self.row += control['h']
        self.variables.append({'name': 'geometry.'+name, 'type': typ, 'value': value, 'control': control})
    ...
    def save(self):
        ...
        patch = {'version': 1, 'goofi': VERSION, 'variables': self.variables, 'variable_groups': {},
...
def recipes():
    ...
    for key in ('phase', 'stretch', 'damping'): p.bind('chord', 'morph', key, 'variables.geometry.'+key)
    p.geometry('trace', 'trace_3d')
    p.bind('trace', 'geometry', 'method', 'variables.geometry.form')
    ...
    p.bind('lineStudy', 'camera', 'yaw', 'variables.geometry.yaw')
    p.node('lightTrace', 'graphics:HarmonicLissajous', (750, 420))
    p.wire('chord', 'harmonic', 'lightTrace', 'harmonics')
    p.bind('lightTrace', 'camera', 'yaw', 'variables.geometry.yaw')
    ...
    for name in ('approach', 'symmetry'): p.bind('plateLight', 'field', name, 'variables.geometry.'+name)
    p.ink('plateLight'); p.bind('ink', 'ink', 'width', 'variables.geometry.lineWidth')
    ...
    p.bind('openWaves', 'field', 'period', 'variables.geometry.period')
    ...
    p.bind('between', 'blend', 'mix', 'variables.geometry.approach')
    ...
    p.bind('mediaStudy', 'field', 'style', 'variables.geometry.style')
    p.ink('between', 'geometry', upload=True)
    p.bind('ink', 'ink', 'style', 'variables.geometry.style')
    ...
    for name in ('affinity', 'temperature'): p.bind('sand', 'transport', name, 'variables.geometry.'+name)
    ...
    p.wire('plate', 'geometry', 'tracer', 'input'); p.bind('tracer', 'transport', 'mixing', 'variables.geometry.flowMix')
    p.node('memory', 'graphics:HarmonicFlow', (1110, 500), seed={'grain': 7.0, 'injection': .002})
    p.wire('tracer', 'geometry', 'memory', 'flow')
    p.bind('memory', 'motion', 'rate', 'variables.geometry.rate'); p.bind('memory', 'seed', 'reset', 'variables.geometry.reset')
    ...
    for tag, y in [('A', 0), ('B', 450)]:
        ...
        p.bind('knot'+tag, 'surface', 'tube_radius', 'variables.geometry.radius')
    ...
    p.bind('knotStudy', 'camera', 'yaw', 'variables.geometry.yaw')
    p.display('knotStudy')
    yield p.save()
    ...
    p.bind('garden', 'geometry', 'method', 'variables.geometry.form')
    p.node('palette', 'signal:BioColors', (380, 500), color={'source': 'tuning', 'method': 'tonotopic', 'fund': 110.0})
    p.wire(p.tuning(), 'out', 'palette', 'input'); p.bind('palette', 'color', 'method', 'variables.geometry.color')
    ...
    p.bind('signal', 'signal', 'shift', 'variables.geometry.signalShift')
    ...
    p.bind('world', 'geometry', 'method', 'variables.geometry.form')
    ...
    p.bind('chord', 'extension', 'amount', 'variables.geometry.extension')
    ...
    for key in ('level', 'transpose'): p.bind('voices', 'sound', key, 'variables.geometry.'+key)
    ...
    for name, value, param in [('textureA', 'sand', 'texture_a'), ('textureB', 'lichen', 'texture_b')]:
        ...
        p.bind('jade', 'material', param, 'variables.geometry.'+name)
    ...
    p.bind('materialMotion', 'lfo', 'amplitude', '0.5 if variables.geometry.textureAuto else 0.0')
    p.bind('materialMotion', 'lfo', 'offset', '0.5 if variables.geometry.textureAuto else variables.geometry.textureMix')
    ...
    for name, value, lo, hi, node, group, param in [
        ...
        p.bind(node, group, param, 'variables.geometry.'+name)
    p.display('jade', 'out')
    yield p.save()
    ...
    for name, param in [('ratios', 'ratios'), ('running', 'running'), ('stepSeconds', 'seconds'), ('glide', 'glide'), ('direction', 'direction')]:
        p.bind('ratios', 'sequence', param, 'variables.geometry.'+name)
    ...
    for name, value, param in [('textureA', 'sand', 'texture_a'), ('textureB', 'spores', 'texture_b')]:
        p.control(name, value, kind='dropdown', options=RELIEF_TEXTURES)
        p.bind('organism', 'material', param, 'variables.geometry.'+name)
    for name, value, lo, hi, node, group, param in [
    ...
    p.bind('ratios', 'chord', 'anchors', 'variables.geometry.anchorChord')
    p.control('squareSymmetry', 'd4_max', kind='dropdown', options=['none', 'd4_max', 'd4_sum'])
    p.bind('field', 'field', 'density_symmetry', 'variables.geometry.squareSymmetry')
    for name, value, param, options in [
        ...
        p.bind('modes', 'modes', param, 'variables.geometry.'+name)
    yield p.save()
