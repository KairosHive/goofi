...
pub static TREE: &[Entry] = &[
    ...
    Group("library", "the node types — what `node add` can build", &[
        ...
        Leaf(Op { name: "save", handler: Effect(arms::library_save), args: "type:string! overwrite:bool", positional: 1,
             doc: "Move a node file OUT of the open patch and into your private library — `$GOOFI_HOME/.goofi/custom/` — where every later patch finds it. Only a node of the patch's own is saved; a library node is already there and a shipped one is not yours. A move, not a copy: the library holds the one source from then on, and a `session save` re-bundles the file into the `.gfi` from there, so a patch still opens on a machine that has no such library. A name the library already holds is REFUSED, naming the file in the way; `--overwrite` replaces that file instead, which `library get` names under `shadowed` before you ask.",
