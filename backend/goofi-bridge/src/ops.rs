...
pub static TREE: &[Entry] = &[
    ...
    Group("session", "the goofi instance as a whole — identity, the open patch, save and load", &[
        ...
        Leaf(Op { name: "state", handler: Read(arms::session_state), args: "", positional: 0,
             ...
             result: "{nodes, links, variables, arrangement} — nodes and variables keyed by id, links a list." }),
    ...
    Group("node", "one node instance — read it, build it, tune it, remove it", &[
        ...
        Group("param", "one param of a node, addressed `group/param`", &[
            ...
            Leaf(Op { name: "request", handler: Effect(arms::node_param_request),
                 args: "node:uid! param:param_addr! request:string!", positional: 2,
                 doc: "One request to a node's own thread, addressed `group/param`: `refresh` asks a refreshable string param (a device or stream picker) to re-enumerate its options — read them back with `node state`; `pulse` fires a pulse param once: a reset, a trigger, a clear. A request holds no value, so it is never an edit of the document, and this reply only says it was dispatched.",
                 result: "{ok: true} — the request was dispatched to the node's own thread" }),
        ]),
    ...
    Group("variable", "the patch variables — what an expression reads as `variables.group.element`", &[
        Leaf(Op { name: "list", handler: Read(arms::variable_list), args: "", positional: 0,
             doc: "Every patch variable — what an expression can read and the variable writes can set — each with the lock that holds it (its own and its group's together), and every group that carries a lock. The `system` group is goofi's own: config-locked for life, and its EPHEMERAL members — `system.goofi_home` and the `system.audio_*` facts the audio engine publishes — are goofi's own value, never saved into a patch.",
             result: "{variables: [{name, type, value, lock: {config, value}, control?, source?}], groups: {group: {lock}}}" }),
        Group("entry", "one variable, addressed `group.element`", &[
            Leaf(Op { name: "add", handler: Write(arms::variable_add),
                 args: "name:string group:string type:string value:any control:json", positional: 1,
                 doc: "Create a patch variable. Give `group` alone to add a float entry with value 0 and the first free entry0/entry1/... name. Otherwise `name`, `type` and `value` are required. `name` is `group.element` — every variable is in a group. `type` is one of float/int/bool/string; a name the patch already holds is refused — `variable entry edit` changes one. `control` makes it a control-panel element: {kind, min, max, step, options, x, y, w, h}, where kind is knob/slider/number/text/toggle/dropdown/paint and must be able to draw the type.",
                 result: "{name, value} — the name and value as stored" }),
            Leaf(Op { name: "edit", handler: Write(arms::variable_edit), args: "name:string! value:any type:string control:json", positional: 1,
                 doc: "Change an existing variable's value, type-coerced to the type it holds. An explicit `type` changes the type, converting the current value when no value is supplied (an unsupported conversion uses an empty value); config-locked entries refuse type changes. A control widget must support the new type. A value-locked variable refuses the edit, and so does an ephemeral one (system.goofi_home, system.audio_*). `control` sets the control-panel widget and its place, `null` clears it, and giving one makes `value` optional — which is what a panel sends when it moves a widget; a config-locked variable refuses it.",
                 result: "{value} — the value as stored, type-coerced" }),
            Leaf(Op { name: "remove", handler: Write(arms::variable_remove), args: "name:string!", positional: 1,
                 doc: "Delete a patch variable. A config-locked one refuses, and a system variable always is.",
                 result: "{removed: true}" }),
            Leaf(Op { name: "source", handler: Write(arms::variable_source), args: "name:string! reference:string! index:int", positional: 2,
                 doc: "Make a variable FOLLOW one producer output, `node.slot`, at that producer's rate: the manager writes the variable on every frame that changes it, and nobody else may set it until the source is cleared with an empty reference. `index` picks one number out of a frame wider than one — a MIDI controller's `cc` is 128 of them — and a frame that holds one number needs none. The reference follows a node rename exactly as a param's does. A config-locked variable refuses; a value-locked one holds its value and takes nothing.",
                 result: "{source: {reference, index} | null}" }),
            Leaf(Op { name: "lock", handler: Write(arms::variable_lock), args: "name:string! config:bool value:bool", positional: 1,
                 doc: "Lock or unlock one variable on its own account: `config` freezes its name, its widget and its place in a group, `value` freezes its value alone. An axis not named keeps what it has, and its group's lock holds it besides. The system group's locks are goofi's own.",
                 result: "{lock: {config, value}} — the variable's own lock as stored" }),
            Leaf(Op { name: "rename", handler: Write(arms::variable_rename), args: "name:string! to:string!", positional: 2,
                 doc: "Rename a variable, and rewrite every expression that reads it. `to` is a full `group.element`, so one op both renames an element and moves it to another group.",
                 result: "{name} — the name as stored" }),
        ]),
        Group("group", "a whole group of variables — a control panel is one", &[
            Leaf(Op { name: "add", handler: Write(arms::variable_group_add), args: "group:string", positional: 1,
                 doc: "Create an empty variables group. Without a name, use the first free group0/group1/... name.",
                 result: "{group} — the created group name" }),
            Leaf(Op { name: "rename", handler: Write(arms::variable_group_rename), args: "from:string! to:string!", positional: 2,
            ...
            Leaf(Op { name: "lock", handler: Write(arms::variable_group_lock), args: "group:string! config:bool value:bool", positional: 1,
    ...
    Group("control", "a control panel: one group of variables drawn as widgets, and the door that edits them", &[
        ...
        Leaf(Op { name: "add", handler: Write(arms::control_add), args: "group:string! kind:string! element:string x:float y:float w:float h:float value:any min:float max:float step:float options:json", positional: 2,
             ...
             result: "{name, control} — the variable's full name and the widget as stored" }),
        ...
        Leaf(Op { name: "remove", handler: Write(arms::control_remove), args: "group:string! element:string!", positional: 2,
             doc: "Delete a widget and the variable under it. Refused by a config lock.",
             result: "{removed: true}" }),
    ...
    Group("record", "capture any node's output to disk, on one clock", &[
        ...
        Leaf(Op { name: "start", handler: Effect(arms::record_start), args: "name:string root:string annotations:json", positional: 1,
             doc: "Begin a recording. `name` names the folder, which otherwise carries the UTC of this moment; `root` overrides the recordings folder for this one recording. Either one absent is read from `variables.record.name` and `variables.record.root`. Refused when nothing is armed, and refused when one already runs.",
