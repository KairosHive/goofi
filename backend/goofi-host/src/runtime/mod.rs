...
use crate::RunPolicy;
use goofi_host_sdk::{Inputs, Node, NodeCtx, Outputs};
...
impl NodeEnv {
    ...
    pub fn detached() -> NodeEnv {
        NodeEnv { engine: "signal", node: None, evaluator: None, time: Arc::new(goofi_core::time::Time::new()) }
    }
}
        ...
        let result = result.and_then(|()| {
            for slot in self.manifest.outputs {
                ...
                match (slot.kind, frame.value()) {
                    ...
                    (goofi_core::SlotType::Texture, _) => return Err(crate::NodeError("TEXTURE output requires a texture submission".into())),
                    (_, goofi_core::Value::Texture(_)) => return Err(crate::NodeError("texture submissions require a TEXTURE output".into())),
                    _ => {}
                }
            }
    ...
    goofi_transport::thread(format!("goofi-{}", manifest.type_name))
        .spawn(move || {
            ...
            if !halt.stopped() {
                ...
                match crate::guard_lifecycle(|| build(&params)) {
                    Ok(node) => NodeRuntime::new(manifest, node, params, transport, env).run_forever(halt.clone()),
                    Err(msg) => transport.report(WireStatus::Health(Status::Fault {
                        fault: Some(NodeFault::Process { msg, since: now_ms() }),
                    })),
                }
            }
        })
