//! The host node's author contract: the `Node` trait a host node implements, the views it is
...
/// A host node's failure, propagated to the health plane rather than panicking.
#[derive(Debug, Clone)]
