//! The native graphics engine behind the `Engine` seam: shader nodes on the GPU, one device, one
//! render thread, no window. The engine (this file) owns the library, the plan and the clock; the
//! render thread (`runtime`) owns the textures and draws; a node's control half (`half`) is a
//! thread of its own, parked on the node's door — the same one the audio engine uses.
...
use crate::half::{self, GraphicsHalf};
use crate::runtime::{self, Runtime, Stats};
use crate::scan::{self, Class, Compiler};
use crate::{plan, producer};
...
pub(crate) struct Instance {
    ...
    pub(crate) program: Option<(Arc<str>, scan::Built)>,
}
...
pub struct GraphicsEngine {
    instance: String,
    pub(crate) time: Arc<goofi_core::time::Time>,
    clock: Clock,
...
pub(crate) fn decls_of(manifest: &'static NodeManifest) -> Vec<ParamDecl> {
...
impl Engine for GraphicsEngine {
    ...
    fn insert(&mut self, uid: Uid, type_name: &str, generation: u64, params: &ParamGroups) -> Option<String> {
        ...
        self.ask(runtime::Cmd::Insert(uid, crate::shader::params_len(manifest)));
