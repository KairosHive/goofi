...
fn a_scheduled_engine_beside_the_signal_one() {
    ...
    assert!(ids.contains(&"signal:LFO") && ids.contains(&"twin:LFO"), "{ids:?}");

    let why = t.refuse("node add", j!({ "type": "LFO" }));
    assert!(why.contains("signal:LFO") && why.contains("twin:LFO"), "names every candidate: {why}");

    let a = t.add("signal:LFO");
    let b = t.add("twin:LFO");
    let doc = t.doc();
    assert_eq!(doc["nodes"][hex(a)]["type"], "signal:LFO");
    assert_eq!(doc["nodes"][hex(b)]["type"], "twin:LFO");
    assert_eq!(doc["nodes"][hex(a)]["name"], "lfo0", "the minted name is the BARE half");
    assert_eq!(doc["nodes"][hex(b)]["name"], "lfo1");
    ...
    assert!(manifest.contains("type: twin:LFO"), "the .gfi records the qualified id: {manifest}");
    t.call("session load", j!({ "content": manifest }));
    let reloaded = t.doc();
    let loaded: Vec<&str> =
        reloaded["nodes"].as_object().unwrap().values().map(|n| n["type"].as_str().unwrap()).collect();
    assert!(loaded.contains(&"signal:LFO") && loaded.contains(&"twin:LFO"), "{loaded:?}");

    let got = t.call("library get", j!({ "type": "twin:LFO" }));
    assert_eq!(got["type"], "twin:LFO");
    ...
    assert_eq!(t.call("library get", j!({ "type": "signal:LFO" }))["editor"], false);
    ...
    assert!(t.refuse("node editor", j!({ "node": hex(a) })).contains("`signal:LFO` has no editor"));
