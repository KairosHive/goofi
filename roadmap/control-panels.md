...
The control panel itself is BUILT (2026-09-05): a group of variables, each carrying a `control`
record, drawn as knobs, sliders and text widgets by the `control` panel type. `variables.group.element` is
...
Picking a control element for a param today means typing `variables.desk.level` into the expression
...
A widget label can be dragged onto a parameter to set `variables.<panel>.<widget>`. In edit mode,
...
A variable now carries a `source` and the manager writes it from a producer's frames; `learn` in
...
- A source naming a node that is not there is silent: the follower has nothing to read and says
  nothing. A param's reference carries an `error` for the same case; the variable's should too.
...
- **The picture leaves the pad as a FRAME.** `Drawing` is a signal node that takes the widget's
  variable the way a knob's value takes one — an expression of `variables.<panel>.<pad>` — decodes the
...
`cl()` was proposed and dropped. A control element IS a variable, so `variables.group.element` is its
