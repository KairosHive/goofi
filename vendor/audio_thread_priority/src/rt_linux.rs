...
pub fn demote_current_thread_from_real_time_internal(
    ...
    let result = unsafe {
        libc::pthread_setschedparam(
            ...
            // Linux requires CAP_SYS_NICE to clear reset-on-fork, even when
            // returning to normal priority. Keep that protection in place.
            rt_priority_handle.thread_info.policy | libc::SCHED_RESET_ON_FORK,
            &param,
        )
    };
    if result != 0 {
        return Err(AudioThreadPriorityError::new_with_inner(
            ...
            Box::new(OSError::from_raw_os_error(result)),
        ));
    }
    Ok(())
}
...
pub fn promote_thread_to_real_time_internal(
    ...
    // Use the permission the system already grants before asking RTKit. RTKit
    // requires a process-wide RLIMIT_RTTIME; installing it when native scheduling
    // is allowed can terminate an otherwise working audio host with SIGXCPU.
    let param = libc::sched_param {
        sched_priority: RT_PRIO_DEFAULT as libc::c_int,
    };
    if unsafe {
    ...
    let error = OSError::last_os_error();
